        ...
        {providers.length > 0 && (
          <div className="external-login-group">
            ...
            {providers.map((provider) => (
              <a
                key={provider.name}
                className={`auth-external-link${provider.name.toLowerCase() === 'google' ? ' auth-external-link--google' : ''}`}
                href={authApi.externalLoginUrl(provider.name, '/', 'login')}
              >
                ...
                {provider.name.toLowerCase() === 'google' ? 'Sign in with Google' : `Sign in with ${provider.displayName}`}
              </a>
            ))}
          </div>
        )}
