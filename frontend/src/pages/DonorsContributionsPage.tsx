...
export function DonorsContributionsPage() {
  ...
  const [thankYouSentTo, setThankYouSentTo] = useState<string | null>(null);
  const [thankYouSendError, setThankYouSendError] = useState<string | null>(null);
  ...
  const openThankYouModal = (supporter: { id: number; displayName: string }) => {
    ...
    setThankYouSentTo(null);
    setThankYouSendError(null);
    void runDraft(supporter.id, 'warm');
  };

  const sendThankYouEmail = async () => {
    if (!thankYouSupporter || !thankYouDraft) return;
    const confirmed = window.confirm(
      `Send this thank-you email to ${thankYouSupporter.name}? It will be delivered from the Kateri notification address.`,
    );
    if (!confirmed) return;
    ...
    setThankYouSendError(null);
    try {
      const result = await donorsContributionsApi.sendThankYou(thankYouSupporter.id, {
        subject: thankYouDraft.subject,
        body: thankYouDraft.body,
      });
      setThankYouSentTo(result.recipient);
    } catch (err) {
      setThankYouSendError(err instanceof Error ? err.message : 'Failed to send the thank-you email.');
    } finally {
      ...
      {thankYouSupporter && (
        ...
        >
          ...
          >
            ...
            {thankYouDraft && !thankYouDrafting && (
              <>
                ...
                <p className="thank-you-model-note">
                  Generated by {thankYouDraft.model} · You can edit freely before sending
                </p>
              </>
            )}
            ...
            {thankYouSendError && (
              <p className="error-text">{thankYouSendError}</p>
            )}
            ...
            <div className="resident-modal-actions">
              ...
              {thankYouDraft && !thankYouDrafting && !thankYouSentTo && (
                <>
                  <button
                    type="button"
                    className="btn-secondary"
